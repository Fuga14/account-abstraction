...
async function main() {
    ...
    let approvalData = await usdcContract.approve.populateTransaction(RANDOM_APPROVER, AMOUNT_TO_APPROVE);
    ...
    console.log(
        `The minimal account nonce before the first tx is ${await provider.getTransactionCount(ZK_MINIMAL_ADDRESS)}`
    );
    ...
    console.log(`The account's nonce after the first tx is ${await provider.getTransactionCount(ZK_MINIMAL_ADDRESS)}`);
}
